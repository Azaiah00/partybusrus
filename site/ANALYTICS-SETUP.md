...
**The verified GA4 stream is deployed to production following the owner-approved release.** The owner authorized creation in their selected signed-in account and completed the agreement step. The Party Bus R Us property contains the Party Bus R Us Website stream for `https://www.partybusrus.com`. Its public Measurement ID, `G-TM8WLPFQC3`, is in `assets/analytics-config.js`. Google Realtime received consented home and quote page views, the initial `quote_step_view`, `session_start` and `first_visit`. This establishes receipt of those events, not every event or mailbox delivery. The site uses direct GA4, without an additional GTM container snippet, Meta Pixel or CallRail installation.
...
Eleven event-scoped custom dimensions were created and verified in the property: `contact_method`, `cta_id`, `placement`, `page_type`, `passenger_band`, `error_code`, `direction`, `step`, `event_type`, `field_name` and `vehicle_id`. These make the corresponding event parameters available for reporting. No click, submission attempt or provider return was promoted to a confirmed-lead key event. Standard reports require production collection and processing; the initial test confirmed Realtime receipt only.

## Consent and storage

With a valid production configuration, the implementation creates no Google loader, GA cookie or event queue before an affirmative choice. A compact nonmodal notice offers equally accessible allow/off buttons. An “Analytics privacy choices” control in the footer lets a visitor change the choice. No consent banner is shown while analytics is unconfigured or on preview hosts. Live DOM checks confirmed no loader before consent, after denial/reload or withdrawal/reload, and exactly one loader after consent. These observations do not independently inspect all third-party network requests or cookies.
...
For consented same-tab quote links, navigation waits for the tag's event-processing callback or an independent 250ms timer, whichever comes first. The timer still navigates if Google is blocked or unresponsive. Modifier/new-tab/download clicks and consent-off visits keep native navigation. This reduces event loss on page unload; a processing callback is not an account-side receipt. The original destination and vehicle preferences are preserved without adding its query string to the event.
